  ...
  ({ className, placeholder, children, error, ...props }, ref) => {
    return (
      <div className={`relative ${error ? "my-2" : ""}`}>
        ...
        ></div>
        <label className="pointer-events-none absolute left-0 -translate-y-full text-xs transition-[transform,font] !duration-150 peer-placeholder-shown:-translate-y-1 peer-placeholder-shown:text-sm peer-focus:-translate-y-full peer-focus:text-xs peer-focus:font-normal peer-focus:text-black motion-reduce:transition-none dark:peer-focus:text-white">
          {placeholder}
        </label>
        {error && (
